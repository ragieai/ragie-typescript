 ...
 */
export function authenticatorsDeleteAuthenticatorConnection(
  client: RagieCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteAuthenticatorConnectionRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/authenticators/{authenticator_id}")(pathParams);
  ...
  const context = {
    ...
    operationID: "DeleteAuthenticatorConnection",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResponseOK$inboundSchema),
