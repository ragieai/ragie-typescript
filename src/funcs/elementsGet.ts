 ...
 */
export function elementsGet(
  client: RagieCore,
  request: operations.GetElementRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ApiDocumentElement,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetElementRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    element_id: encodeSimple("element_id", payload.element_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/elements/{element_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ApiDocumentElement$inboundSchema),
